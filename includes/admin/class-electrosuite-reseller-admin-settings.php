...
class ElectroSuite_Reseller_Admin_Settings {
	...
	public static function output() {
		...
		wp_enqueue_script( 'electrosuite_reseller_settings', ElectroSuite_Reseller()->plugin_url() . '/assets/js/admin/settings.min.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'iris' ), ElectroSuite_Reseller()->version, true );
		...
		foreach ( $options as $value ) {
			...
			switch ( $type ) {
				...
				case "radio" :
					...
					if ( isset( $_POST[$value['id']] ) ) {
						$option_value = electrosuite_reseller_clean( stripslashes( $_POST[ $value['id'] ] ) );
					}
					else {
						$option_value = '';
					}
				...
				case "multi_select_countries" :
					...
					// Get countries array
					if ( isset( $_POST[ $value['id'] ] ) ) {
						$selected_countries = array_map( 'electrosuite_reseller_clean', array_map( 'stripslashes', (array) $_POST[ $value['id'] ] ) );
					}
					else {
				...
				case "image_width" :
					...
					if ( isset( $_POST[$value['id'] ]['width'] ) ) {
						...
						$update_options[ $value['id'] ]['width']  = electrosuite_reseller_clean( stripslashes( $_POST[ $value['id'] ]['width'] ) );
						$update_options[ $value['id'] ]['height'] = electrosuite_reseller_clean( stripslashes( $_POST[ $value['id'] ]['height'] ) );
