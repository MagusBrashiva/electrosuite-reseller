<?php
class ElectroSuite_Reseller_Transifex_Stats {
	...
	function __construct( ) {
		$this->project_slug  = ElectroSuite_Reseller()->transifex_project_slug;
		$this->resource_slug = ElectroSuite_Reseller()->transifex_resources_slug;
		$this->cache_time    = 3600;
		$this->api = new ElectroSuite_Reseller_Transifex_API( $this->cache_time );
	}
	...
	function maybe_display_error( $response ) {
		...
		if ( ! $response ) {
			$error = __('No results', ELECTROSUITE_RESELLER_TEXT_DOMAIN );
		}
	...
	public function display_translations_progress() {
		...
		$languages = apply_filters( 'electrosuite_reseller_transifex_stats', $languages, $project );
			...
			<li class="clearfix">
				...
				<div class="statbar">
					...
					<div class="go_translate">
						<a target="_blank" href="https://www.transifex.com/projects/p/<?php echo ElectroSuite_Reseller()->transifex_project_slug; ?>/translate/#<?php echo $language_code; ?>"><?php _e( 'Translate', ELECTROSUITE_RESELLER_TEXT_DOMAIN ); ?></a>
					</div>
				</div>
			</li>
			<?php
